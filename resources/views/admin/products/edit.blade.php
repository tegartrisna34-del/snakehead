    ...
    <div class="max-w-4xl mx-auto">
        <header class="flex justify-between items-center mb-10">
            <div>
                ...
                <h2 class="text-3xl font-bold">Edit Produk: <span class="text-emerald-500">{{ $product->name }}</span></h2>
            </div>
        </header>
            ...
            <div class="glass p-8 rounded-[2rem] space-y-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="name" value="{{ $product->name }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 transition text-white">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <select name="category_id" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 transition text-white appearance-none">
                            @foreach($categories as $category)
                                <option value="{{ $category->id }}" class="bg-[#0c1117]" {{ $product->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="price" value="{{ $product->price }}" required class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 transition text-white">
                    </div>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="flex space-x-4">
                            <input type="number" name="stock" value="{{ $product->stock }}" required class="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 transition text-white">
                            <input type="text" name="unit" value="{{ $product->unit }}" class="w-24 bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 transition text-white">
                        </div>
                    </div>
                </div>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="description" required rows="4" class="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 focus:outline-none focus:border-emerald-500/50 transition text-white">{{ $product->description }}</textarea>
                </div>

                <!-- Gambar -->
                <div class="space-y-2">
                    <label class="text-sm font-bold text-gray-400 uppercase tracking-widest">Foto Produk (Biarkan kosong jika tidak diganti)</label>
                    
                    @if($product->image)
                    <div class="mb-4">
                        <p class="text-xs text-gray-500 mb-2">Foto Saat Ini:</p>
                        <img src="{{ asset($product->image) }}" class="h-32 rounded-xl border border-white/10">
                    </div>
                    @endif
                    ...
                    <div class="relative group">
                        ...
                        <div class="absolute inset-x-0 top-0 h-full flex flex-col items-center justify-center pointer-events-none" id="placeholder">
                            ...
                            <span class="text-sm">Klik untuk mengganti gambar</span>
                        </div>
                    </div>
                </div>
            </div>
